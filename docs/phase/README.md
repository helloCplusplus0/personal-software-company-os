...
- 根级当前状态：`phase09_template_reuse_derived_intelligence_foundation` 已完成正式交付、统一验收与收口，当前保留为最近完成的正式支撑能力 phase；`phase08_operating_review_loop_foundation` 继续保留为最近完成的正式业务 phase
...
- 当前暂无新的正式 `/plan` 入口；后续 `dry-run` 仍为独立验收闸，只允许在直接承接 `phase09-11` 验收结论、`phase09` 根级收口结论、`phase08` 根级收口结论与当前根级真相源后建立
- 在正式入口建立前，只保留 `dry-run` 进入条件，不预设 `mvp0.4+`、新的 phase 名称或范围拆分
