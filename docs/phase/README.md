...
- 根级当前状态：`phase08_operating_review_loop_foundation` 已完成正式交付、验收与收口
...
- 后续正式 phase 入口只允许在直接承接 `phase08-11` 验收结论、`phase08` 根级收口结论与当前根级真相源后建立（不得预设新的 phase 名称）
...
- 后续支撑能力 phase / dry-run phase 在正式入口建立前，只允许以进入条件形式出现，不得提前命名未建立的新阶段
