...
//   - ErrRepositoryNotFound / ErrProductNotFound / ErrModuleNotFound → 404
//   - ErrDuplicateBinding / ErrDuplicateMapping → 409
//   - ErrInvalidInput / ErrInvalidStatus / ErrProductNotActive / ErrModuleNotActive → 400
//   - 其他 → 500
func writeError(w http.ResponseWriter, err error) {
	switch {
	case errors.Is(err, repositorybinding.ErrRepositoryNotFound),
		errors.Is(err, repositorybinding.ErrProductNotFound),
		errors.Is(err, repositorybinding.ErrModuleNotFound):
		writeJSON(w, http.StatusNotFound, errorResponse{Error: err.Error()})
	case errors.Is(err, repositorybinding.ErrDuplicateBinding),
		errors.Is(err, repositorybinding.ErrDuplicateMapping):
		writeJSON(w, http.StatusConflict, errorResponse{Error: err.Error()})
	case errors.Is(err, repositorybinding.ErrInvalidInput),
