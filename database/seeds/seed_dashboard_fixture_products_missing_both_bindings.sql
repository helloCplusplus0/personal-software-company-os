-- seed_dashboard_fixture_products_missing_both_bindings.sql
-- phase05-12 验收 fixture 9：products-missing-both-bindings（CTA 6 扩展）
--
-- 用途：
--   建立 Dashboard CTA 6 验收状态：product missing both bindings → Product Detail。
--   FeedbackSignalRead 产出 PRODUCT_MISSING_BOTH_BINDINGS 信号。
--
-- 数据状态（phase05-09 spec §"fixture 9 — products-missing-both-bindings"）：
--   - 1 条目标 product：同时缺少 product_modules 与 product_repositories 记录
--   - 1 条对照 product：已完整绑定
--   - modules 与 repositories 必须存在（使缺口判定为"product 缺绑定"而非"系统缺实体"）
--   - 不存在 pending_decision_signals
--   - FeedbackSignalRead 产出 PRODUCT_MISSING_BOTH_BINDINGS 信号
--
-- 依赖：reset_dashboard_acceptance.sh --fixture products-missing-both-bindings 已执行清空操作
-- 幂等：使用 ON CONFLICT DO NOTHING，可重复执行。
-- 使用方式：
--   ./database/scripts/reset_dashboard_acceptance.sh --fixture products-missing-both-bindings
...
-- ============================================================================
-- 产品（2 条）：Product A（目标，缺双绑定）/ Product B（对照，完整绑定）
-- ============================================================================
INSERT INTO products (name, description, status) VALUES
  ('Product A', 'Dashboard fixture：目标产品，同时缺模块与仓库绑定', 'active'),
