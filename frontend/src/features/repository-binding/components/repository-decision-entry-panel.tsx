...
interface RepositoryDecisionEntryPanelProps {
...
/**
 * RepositoryDecisionEntryPanel — Repository Detail 的 Decision 入口面板
 *
 * phase10-10 §"Repository Detail Decision 入口面板"：
 * - 承接"为当前 Repository 记录决策"与"查看当前 Repository 相关决策"两类正式入口
 * - "记录决策" → 导航到带 sourceRepositoryId / sourceRepositoryName 的 /decisions/new
 * - "查看全部" → 导航到 /decisions
 */
export function RepositoryDecisionEntryPanel({
  decisionLinks,
  decisionDetailSearch,
  isLoading = false,
  isError = false,
}: RepositoryDecisionEntryPanelProps) {
  return (
    <Card>
      <CardHeader>
