...
import { useRepositoryModuleCandidatesRead } from '../data/use-repository-module-candidates-read'
import { useMapModuleToRepository } from '../application/use-map-module-to-repository'
...
export function RepositoryModuleMappingPanel({
  ...
  const { data: candidates, isLoading: candidatesLoading, isError: candidatesError } = useRepositoryModuleCandidatesRead(repositoryId)
  ...
  const mutation = useMapModuleToRepository()

  /** 包装映射操作以添加页面级 toast 与回调 */
  const handleBind = () => {
    mutation.mutate(
      { repositoryId, moduleId: selectedModuleId },
      {
        onSuccess: () => {
